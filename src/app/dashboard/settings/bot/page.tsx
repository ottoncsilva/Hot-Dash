...
import SectionRow from "@/components/telegram/bot/SectionRow";
import { IconPayments, IconSparkle } from "@/components/icons";
...
export default function BotSettingsPage() {
  ...
  // O RESUMO de cada linha é o estado salvo, em uma frase — é o que permite
  // ver como as duas estão sem precisar abrir nenhuma.
  const resumoPreco = preco.enabled
    ? `Ligado · até ${preco.cents} centavo(s) · ${
        preco.direction === "up" ? "para cima" : preco.direction === "down" ? "para baixo" : "aleatório"
      }`
    : "Desligado — todo mundo paga o mesmo valor";
  const quantasCores = roles.filter((r) => estilos[r.key]).length;
  const resumoCores =
    quantasCores > 0
      ? `${quantasCores} de ${roles.length} papéis com cor própria`
  ...
  return (
    <div className="page-narrow">
      ...
      <p className="mb-3 mt-3 text-xs leading-relaxed text-zinc-500">
        As duas valem para <b>todas as modelos</b> de uma vez — por isso moram aqui, e não na tela
        de cada bot.
      </p>

      {/* Duas linhas SEPARADAS e retráteis, como as da tela do bot. No Bobz os
          dois assuntos vivem num cartão só; aqui não, porque são decisões
          independentes: mexer no preço não tem nada a ver com escolher cor. */}
      <div className="space-y-3">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              ...
              <div>
                ...
                <p className="mt-1 text-[11px] text-zinc-500">
                  No aleatório o sentido também vem do ID, então não muda entre as tentativas do
                  mesmo cliente.
                </p>
              </div>
            </div>
        ...
        <SectionRow
          ...
          summary={resumoCores}
          status={quantasCores > 0 ? { label: `${quantasCores} com cor`, tone: "ok" } : undefined}
        >
          <p className="text-xs leading-relaxed text-zinc-400">
            Cor dos botões que o bot mostra dentro do Telegram. Cada papel do fluxo tem a sua,
            porque a intenção muda: a lista de planos pede destaque, copiar a chave é auxiliar, e o
            acesso ao VIP depois do pagamento merece o verde.
          </p>
