...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!["openai", "gemini", "grok", "openrouter", "magnific", "kling", "byteplus", "nudenet"].includes(provider)) throw new ApiError(400, "Provedor inválido.");
    ...
    const result = await testAiProviderKey(testProvider, apiKey, { baseUrl: body.baseUrl });
    return NextResponse.json({ connected: result.ok, message: result.message });
  } catch (err) {
